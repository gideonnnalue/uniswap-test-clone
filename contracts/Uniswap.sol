...
contract Uniswap {
    ...
    function swapEthToToken(string memory tokenName) public payable returns (uint) {
        ...
        uint outputValue = (inputValue / ethValue) * 10 ** 18; // Convert to 18 decimal
    ...
    function swapTokenToToken(string memory srcTokenName, string memory destToken, uint _amount) public {
        require(tokenInstanceMap[srcTokenName].transferFrom(msg.sender, address(this), _amount));
        require(tokenInstanceMap[destToken].transfer(msg.sender, _amount));
    }
